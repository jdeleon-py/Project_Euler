# Find the thirteen adjacent digits in the 1000-digit 
# number that have the greatest product. What is the value of this product?

class SeriesCalculation: # analyzes left to right of an input number 

    def __init__(self, input_num, len_analyze):
        self.start_arr = 0
        self.len = len_analyze
        self.input = input_num 
        
        self.arr = [int(i) for i in str(self.input)]
        self.sub_arr = self.arr[self.start_arr:self.len]

        self.maximum = 0

    def adjacent_product_analyze(self): # returns greatest product created from len_analyze adjacent digits
        counter = 1
        while True:
            if self.len > len(self.arr):
                break

            for i in range(0, len(self.sub_arr)):
                counter = self.sub_arr[i] * counter
                if counter > self.maximum:
                    self.maximum = counter
            
            counter = 1
            self.start_arr += 1
            self.len += 1
            self.sub_arr = self.arr[self.start_arr:self.len]
            continue

        return self.maximum

    def adjacent_sum_analyze(self): # returns greatest sum created from len_analyze adjacent digits
        counter = 0
        while True:
            if self.len > len(self.arr):
                break

            for i in range(0, len(self.sub_arr)):
                counter = self.sub_arr[i] + counter
                if counter > self.maximum:
                    self.maximum = counter
            
            counter = 1
            self.start_arr += 1
            self.len += 1
            self.sub_arr = self.arr[self.start_arr:self.len]
            continue

        return self.maximum


class GridProductCalculation(SeriesCalculation): # instead of input_num being a number, input_num is a matrix

    def __init__(self, input_num, len_analyze):
        SeriesCalculation.__init__(self, input_num, len_analyze)

        # modify input_num to be both a number where every 
        # two digits = one sub-number (12, 45, 06, 00, 93) and a matrix

    def create_grid(self):
        pass

    def check_product_lr(self):
        return SeriesCalculation.adjacent_product_analyze()




x = 

test1 = SeriesCalculation(x, 4)
print(test1.adjacent_product_analyze())

test2 = SeriesCalculation(x, 13)
print(test2.adjacent_product_analyze())
